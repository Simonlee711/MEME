...
class Classifier(nn.Module):
    def __init__(self, input_dim, num_labels=3):
        ...
        self.loss_func = nn.BCEWithLogitsLoss() # Change this if it becomes more than binary classification
    ...
    def binary_cross_entropy(self, y_true, y_pred):
        ...
        for i, label in enumerate(['further_discharge', "mortality", "ICU"]):  
            loss -= (y_true[:, i] * torch.log(y_pred[:, i]) +
                     (1 - y_true[:, i]) * torch.log(1 - y_pred[:, i]))
        return loss.mean()
...
class ED_classifier(nn.Module):
    def __init__(self, checkpoint, num_labels=2, input_dim=768, modalities=None, freeze=True):
        super(ED_classifier, self).__init__()
        self.encoder = ED_encoder(checkpoint=checkpoint, num_labels=num_labels, freeze=freeze)
    ...
    def forward(self, input_ids, attention_mask, label=None):
        ...
        for modality in range(self.modalities):
            ...
            min_batch_size = min(min_batch_size, embed.size(0)) # Find the smallest batch size

        # Ensuring all embeddings have the same batch size
        uniform_embeddings = [e[:min_batch_size] for e in embedding]

        # Concatenating embeddings
        unified_embedding = torch.cat(uniform_embeddings, 1)  # concatenates embeddings on the second dimension
